...
const CSS = `
/* Themed to the Deck (the customer's project view) — --dv-* tokens with matching fallbacks. */
.pann{position:fixed;inset:0;z-index:9100;display:flex;align-items:center;justify-content:center;font-family:var(--dv-font,'Hanken Grotesk',system-ui,sans-serif)}
.pann-bg{position:fixed;inset:0;background:rgba(11,15,26,.74);animation:pannf .25s ease}
.pann-card{position:relative;width:min(370px,92vw);background:var(--dv-raise,#FBFBFA);border:1px solid var(--dv-line,#E4E4DF);border-radius:20px;border-top:5px solid var(--dv-gold,#C9A96E);box-shadow:0 28px 70px rgba(0,0,0,.5);padding:26px 24px 18px;text-align:center;animation:pannp .34s cubic-bezier(.2,.85,.3,1.1)}
...
.pann-x{position:absolute;top:12px;right:12px;width:28px;height:28px;border:none;background:var(--dv-paper,#F4F4F2);border-radius:9px;color:var(--dv-meta,#787D84);font-size:.82rem;cursor:pointer;line-height:1}
...
.pann-go:hover{filter:brightness(1.04)}
.pann-later{margin-top:8px;width:100%;height:34px;border:none;background:none;color:var(--dv-faint,#A1A6AC);font-size:.84rem;font-weight:600;cursor:pointer;font-family:inherit}
.pann-later:hover{color:var(--dv-meta,#787D84)}
`;
