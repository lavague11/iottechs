...
.icl-pricebtn{height:32px;padding:0 13px;border-radius:8px;border:1px solid #d9d4ca;background:#fff;color:#4a5270;font-size:.76rem;font-weight:800;cursor:pointer;font-family:inherit;white-space:nowrap}
...
.icl-sum-v{font-size:1.12rem;font-weight:800;color:#C9A96E;display:flex;align-items:center;gap:5px}
.icl-sum-sub{font-size:.68rem;color:#8a93a8}
.icl-sum-unit{font-size:.72rem;font-weight:700;color:#8a93a8;margin-left:1px}
.icl-hrs-in{width:58px;height:28px;border:1px solid #3a4260;border-radius:7px;background:#151a2d;color:#C9A96E;font-size:1rem;font-weight:800;text-align:center;padding:0 4px;font-family:inherit;outline:none}
.icl-hrs-in.wide{width:82px;text-align:left}
.icl-hrs-in:focus{border-color:#C9A96E}
.icl-paywrap{display:flex;align-items:center;gap:8px;flex-shrink:0}
.icl-earn{font-size:.82rem;font-weight:800;color:#2f7d5a;font-variant-numeric:tabular-nums;white-space:nowrap}
.icl-earn.done{color:#1d7a3a}
...
.icl-pp-earned{font-size:.98rem;font-weight:800;color:#0B0F1A}
.icl-pp-of{font-size:.78rem;font-weight:600;color:#6f7686}
.icl-pp-pct{font-size:1.05rem;font-weight:800;color:#1d7a3a}
.icl-pp-bar{height:8px;border-radius:100px;background:#d6ecdd;overflow:hidden}
.icl-pp-fill{height:100%;background:linear-gradient(90deg,#5FB88A,#2f7d5a);transition:width .35s ease}
.icl-pp-stats{display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap;font-size:.76rem;color:#4a5270}
.icl-pp-stats b{color:#0B0F1A;font-weight:800}
.icl-eod{margin-top:14px;border-top:1px dashed #d9d4ca;padding-top:12px}
.icl-eod-btn{height:38px;padding:0 18px;border:none;border-radius:9px;background:#0B0F1A;color:#C9A96E;font-size:.82rem;font-weight:800;cursor:pointer;font-family:inherit}
...
.icl-eod-date{font-size:.8rem;font-weight:800;color:#0B0F1A}
.icl-eod-time{font-weight:600;color:#8a93a8}
.icl-eod-meta{font-size:.76rem;color:#2f7d5a;font-weight:700}
.icl-eod-cum{color:#8a93a8;font-weight:600}
.icl-eod-del{width:26px;height:26px;border:1px solid #e2ddd2;background:#fff;border-radius:7px;color:#a8442f;cursor:pointer;font-size:.72rem;flex-shrink:0}
...
.icl-eod-yes{height:24px;padding:0 9px;border:none;border-radius:6px;background:#a8442f;color:#fff;font-size:.68rem;font-weight:800;cursor:pointer;font-family:inherit}
...
.icl-eod-for{display:flex;align-items:center;gap:6px;font-size:.78rem;color:#6f7686;font-weight:600}
.icl-eod-date-in{height:34px;border:1px solid #d9d4ca;border-radius:8px;padding:0 9px;font-size:.8rem;font-family:inherit;color:#0B0F1A;background:#fff;outline:none}
.icl-eod-date-in:focus{border-color:#2f7d5a}
.icl-bytech{margin-top:14px;background:#0B0F1A;border-radius:11px;padding:12px 14px;display:flex;flex-direction:column;gap:6px}
...
.icl-bytech-row b{color:#C9A96E;font-weight:800;font-variant-numeric:tabular-nums}
...
.icl-wolog-main{font-size:.8rem;color:#0B0F1A}
.icl-wolog-main b{font-weight:800}
.icl-wolog-time{font-size:.72rem;color:#8a93a8}
.icl-pay-edit{display:flex;align-items:center;gap:1px;font-size:.84rem;font-weight:800;color:#0B0F1A;flex-shrink:0}
.icl-pay-in{width:58px;height:30px;border:1px solid #d9d4ca;border-radius:7px;background:#fff;color:#0B0F1A;font-size:.82rem;font-weight:800;text-align:right;padding:0 6px;font-family:inherit;outline:none;font-variant-numeric:tabular-nums}
.icl-pay-in:focus{border-color:#2f7d5a}
.icl-bulk{display:flex;align-items:center;gap:7px;flex-wrap:wrap;margin:2px 0 4px}
.icl-bulk-lbl{font-size:.72rem;font-weight:800;color:#6f7686;letter-spacing:.03em}
.icl-bulk-btn{height:28px;padding:0 12px;border-radius:100px;border:1px solid #d9d4ca;background:#fff;color:#4a5270;font-size:.74rem;font-weight:700;cursor:pointer;font-family:inherit}
.icl-bulk-btn:hover{border-color:#C9A96E;color:#8a6d2f;background:#fff8ee}
.icl-bulk-btn.all{border-color:#2f7d5a;color:#1d7a3a;background:#f2f9f4;font-weight:800}
.icl-bulk-btn.all:hover{background:#e7f6ec}
.icl-bulk-btn.reset{border-color:#e0b0a8;color:#a8442f;background:#fff}
...
.icl-cf-yes{height:30px;padding:0 14px;border:none;border-radius:8px;background:#c0392b;color:#fff;font-size:.76rem;font-weight:800;cursor:pointer;font-family:inherit}
.icl-cf-yes:hover{filter:brightness(1.08)}
.icl-cf-no{height:30px;padding:0 12px;border:1px solid #d9d4ca;border-radius:8px;background:#fff;color:#4a5270;font-size:.76rem;font-weight:700;cursor:pointer;font-family:inherit}
.icl-addbtn.req{border-color:#c9b58a;background:#fff8ee;color:#8a6d2f}
.icl-addbtn.req:hover{border-color:#C9A96E;background:#fdf3e0}
...
.icl-req-name{font-size:.86rem;font-weight:800;color:#0B0F1A}
...
.icl-req-meta{font-size:.76rem;color:#6f7686}
.icl-req-act{display:flex;align-items:center;gap:7px;flex-shrink:0}
.icl-req-yes{height:32px;padding:0 14px;border:none;border-radius:8px;background:#2f7d5a;color:#fff;font-size:.78rem;font-weight:800;cursor:pointer;font-family:inherit}
...
.icl-row.done{background:#f2f9f4;border-color:#bfe0c9;border-left-color:#2f7d5a}
...
.icl-name{font-size:.88rem;font-weight:700;color:#0B0F1A;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.icl-step{font-size:.78rem;color:#6f7686}
.icl-step.done{color:#1d7a3a;font-weight:700}
.icl-step b{color:#8a6d2f;font-weight:800}
.icl-step-done{color:#1d7a3a;font-weight:700}
.icl-pay{font-size:.84rem;font-weight:800;color:#0B0F1A;flex-shrink:0;font-variant-numeric:tabular-nums}
.icl-acts{display:flex;align-items:center;gap:4px;flex-shrink:0}
.icl-noteb,.icl-undo,.icl-del{width:32px;height:32px;border:1px solid #e2ddd2;background:#fff;border-radius:8px;color:#6f7686;cursor:pointer;display:flex;align-items:center;justify-content:center}
.icl-noteb:hover,.icl-undo:hover{border-color:var(--icl-c);color:#0B0F1A}
.icl-noteb.has{background:#fff8ee;border-color:#e5d3a1;color:#8a6d2f}
.icl-del:hover{border-color:#e0b0a8;color:#a8442f;background:#fbeceb}
.icl-note{padding:0 12px 11px 55px}
.icl-note-in{width:100%;border:1px solid #d9d4ca;border-radius:8px;background:#fdfcf9;color:#0B0F1A;padding:8px 10px;font-size:.8rem;font-family:inherit;outline:none;resize:vertical}
...
.icl-note-ro{font-size:.78rem;color:#6f7686}
...
.icl-add-in{flex:2;min-width:180px;height:38px;border:1px solid #d9d4ca;border-radius:8px;padding:0 11px;font-size:.82rem;font-family:inherit;outline:none}
.icl-add-sel,.icl-add-pay{height:38px;border:1px solid #d9d4ca;border-radius:8px;padding:0 10px;font-size:.82rem;font-family:inherit;outline:none;background:#fff}
.icl-add-pay{width:100px}
.icl-add-save{height:38px;padding:0 18px;border:none;border-radius:8px;background:#2f7d5a;color:#fff;font-size:.8rem;font-weight:800;cursor:pointer;font-family:inherit}
.icl-add-save:disabled{opacity:.5;cursor:default}
.icl-add-cancel{height:38px;padding:0 14px;border:1px solid #d9d4ca;border-radius:8px;background:#fff;color:#4a5270;font-size:.8rem;font-weight:700;cursor:pointer;font-family:inherit}
.icl-empty{padding:20px;text-align:center;color:#6f7686;font-size:.86rem}
.icl-ro{margin-top:12px;font-size:.74rem;color:#6f7686;font-style:italic}
`;
