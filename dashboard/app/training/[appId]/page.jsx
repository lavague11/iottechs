...
import { resolveApplicationRef, getApplicationTraining } from "../../../lib/db";
...
export function generateMetadata() {
  return { title: "IOT TECHS · Training", description: "Secure access — authorized only.", robots: { index: false, follow: false },
...
export default async function TrainingPage({ params }) {
  ...
  if (!auth.ok) {
    return <SvcGate svcId={app.app_id} endpoint="/api/app-pin-check" idField="appId" kicker="Training"
      grantedLine="Opening your training" altHref="/apply" altLabel="Start a new application" />;
  }

  const training = getApplicationTraining(app.app_id);
  return (
    <TrainingClient
      appId={app.app_id}
