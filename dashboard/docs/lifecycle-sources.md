...
| *Phase 2* (2026-09-22) | **Install-complete + QC sign-off facts.** `getToolMeta` derives `install` (every line item at its last step, from the `install` + `addendum` blobs against the active proposal) and `qc` (pass state + content fingerprint) via the shared pure model `lib/install-checklist-model.js` (`installItemsFromProposal`, `installProgress`, `qcProgress` — the QC checklist now derives its items from it too). `buildStageFacts` adds `install_done`, `qc_passed`, `qc_manager_approved`, `qc_customer_signed`; `STAGE_FLOW.install[0]`, `qc[0]`, `qc[1]` gained `check()`s → **13 of 16 steps gate**. Sign-offs reuse `stage_acceptances` (`qc_manager` admin/manager, `qc_customer` customer-only, each fingerprint-bound to the QC checklist and only accepted once every device passes; logged `approve`/`sign`). UI: `<QcSignoff>` in the internal QC footer (Approve) and a customer "Walkthrough" card in Closeout (Confirm), rendered only once QC passes. `page.jsx` ships the four facts so every role's client gate agrees with the server. | Phase-2 item from §4 |
...
| *Ledger P4* (2026-09-23) | **Diff before re-sign + addendum binding.** `proposalDiff(prevSigned, cur)` (lib/proposal.js) compares the fingerprinted meaning — Added / Changed / Removed per option, discount, PCP, totals, tax, deposit; `getLatestSignedVersion` (frozen `signed_payload`) feeds `getProposalDiffAction`; the customer view renders a structured "What changed v1 → v2" block above the acceptance box and the button reads **Review & Sign**. Prior signed versions untouched; deposit anchoring unchanged (`projectFinancials`). Addenda: `addendumFingerprint` (title / items name-qty-price / discount; techPay excluded) stored as `signedFingerprint` by the new customer-only `signAddendumAction`; `getApprovedAddons` honors only fingerprint-current approvals; office edits to an approved add-on drop it to pending "Changed — re-sign" with a `history` entry; non-office blob writes can't flip approval state. Notifications: signature invalidated by revision → customer; add-on needs signature → customer; add-on signed → office. | closes P4 |
...
| *Review fixes* (2026-09-23) | Independent code review of the gate/permission/signature diff surfaced boundary gaps, all closed: **server-side stripping by capability** in `sanitizeToolRead` (sales no longer receives payouts/rate book; `getRatesAction` gated on `install.pay.view`); **`/api/media`** now requires an upload role, scopes PIN grants to their project, whitelists project `kind`s and confines applicants to `compliance:*` on their own id (kind `install` is a lifecycle fact, so it had to be hardened); **RSVP fact** counts the customer's confirmation only (a tech's own RSVP no longer satisfies a customer requirement); **addendum writes**: a non-office writer can't create a pre-signed add-on, non-pricing roles keep stored prices (or land unpriced), the legacy customer blob path now stamps `signedFingerprint`, and a legacy approval with no fingerprint is compared against the stored content so an edit still forces a re-sign; **proposal diff** hidden from tech/vendor (retail); **vendor** gets only `tracking` meta; **logged-in techs** must be on the project (roster / crew / WO signer) to list or flag issues; **mark-fixed** matches `assigned_to_id` before display name; `buildStageFacts` builds tool meta once; **QC covers approved add-on devices** and the per-item fingerprint now includes device identity + install state. | post-review |
...
The map sums all 16 D6 rows; `phaseGate` only enforces the rows that have a `check()` — 10 at trace time, **13 after Phase 2** (inquiry 1 · site_survey 1 · proposal 2 · approval_deposit 4 · schedule 1 · install 1 · qc 2 · payment 1). Same constant, deliberate filter (`blockingReqs`). Not a bug — but the map did not distinguish enforceable from advisory steps (now shows "N steps · M gated").
